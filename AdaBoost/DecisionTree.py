...
class DecisionTree():
    ...
    def information_gain(self, parent: np.ndarray, left: np.ndarray, right: np.ndarray) -> float:
        ...
        """
        information_gain = 0
        parent_weight = np.sum(parent[:, 1]) / np.sum(self.sample_weight)
        parent_entropy = self.entropy(parent)
        left_weight = np.sum(left[:, 1]) / np.sum(parent[:, 1])
        left_entropy = self.entropy(left)
        right_weight = np.sum(right[:, 1]) / np.sum(parent[:, 1])
        right_entropy = self.entropy(right)

        information_gain = parent_weight * (parent_entropy - left_weight * left_entropy - right_weight * right_entropy)
    ...
    def find_best_split(self, feature: int, data: np.ndarray) -> dict:
        ...
        for threshold in thresholds[1:]:
            ...
            if len(left) and len(right):
                y, y_left, y_right = data[:, -2:], left[:, -2:], right[:, -2:]
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, sample_weight: np.ndarray=None):
        ...
        Args:
            ...
            y: The target values.
            sample_weight: The weights of data points
        """
        if np.any(sample_weight):
            self.sample_weight = sample_weight
            y = np.concatenate((y.reshape(-1, 1), sample_weight.reshape(-1, 1)), axis=1)
